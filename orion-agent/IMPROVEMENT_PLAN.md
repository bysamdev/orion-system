...
| A.3 ✅ | Rate limiting + alerta em `machine-login` e `heartbeat` | C2 | médio | baixo | Implementado e testado (`lib/ratelimit.go`, janela deslizante em memória). **Limitação documentada no código**: backend é serverless (Vercel) — instâncias não compartilham memória, então o limite real pode ser N× o configurado. Correção completa exigiria contador centralizado (schema novo ou KV externo), fora de escopo. "Alerta" = log estruturado, visível nos logs de runtime do Vercel — não há canal de notificação (e-mail/Slack) configurado |
| A.4 ⚠️ | Reduzir privilégio do serviço (conta virtual em vez de `LocalSystem`) | C4 | médio | **alto** | Código implementado: `UserName: NT SERVICE\OrionAgent`. Corrigida a interação com A.6 (ACL do diretório de identidade agora reaplicada em toda gravação, não só na criação — senão instalações existentes ficariam sem acesso ao próprio token). **Não validado em máquina real** — continua sendo o requisito antes de instalar em produção |
...
| A.9 ✅ | Remover fallback "primeira empresa do banco" | A11 | baixo | médio | Implementado. `db.FirstCompanyID` removida (ficou órfã). **Mudança de comportamento real em produção**: heartbeats que hoje dependem desse fallback (chave global sem domínio reconhecido) passam a ser rejeitados com 400 a partir deste deploy, em vez de misassign silencioso |
...
| A.12 📄 | Enrollment com certificado por máquina (mTLS) | C3, C5 | **alto** | **alto** | Documento de design escrito, aprovado como primeiro passo ("documento antes de código"): `MTLS-ENROLLMENT-OPTIONS.md`. **Achado principal**: Vercel não expõe verificação de certificado de cliente para Serverless/Edge Functions (confirmado na documentação oficial de TLS) — mTLS de verdade não roda no backend atual sem decisão de infraestrutura separada (proxy próprio ou LB com suporte a mTLS de origin). Recomendação: tratar o risco concreto já documentado (token estático de `machine-login` em query string) como passo imediato independente de infra, e represar mTLS até a decisão de infraestrutura. Nenhum código implementado |
| A.13 ✅ | SID do usuário via API do Windows em vez de `os.Getenv` | C5 | alto | médio | Implementado e testado. `usuarioDaSessaoAtiva` (`collector/session_windows.go`) resolve domínio/usuário/SID da sessão de CONSOLE ativa via WTS (`WTSQuerySessionInformationW` — não coberta por `x/sys/windows` v0.20.0, vinculada diretamente via `wtsapi32.dll`; SID resolvido com `windows.LookupSID`). Verificado empiricamente nesta máquina de desenvolvimento (sessão real → domínio/usuário/SID corretos). Cai para `os.Getenv` (`identidadeViaEnv`, testado isoladamente) só quando não há sessão de console ativa. Payload/heartbeat ganharam `current_user_sid`; backend persiste em `machines.current_user_sid` (migração `20260812130000`, coluna nullable, **não aplicada** — só escrita em disco). SID é dado informativo de inventário, nunca usado para auth (ver `SECURITY-AUTO-PROVISIONING.md` §4.2/§4.4). **Efeito colateral corrigido nos testes**: WTS agora tem prioridade sobre env vars, então 3 testes que antes manipulavam `USERNAME`/`USERDOMAIN` e esperavam vê-los refletidos em `Collect()` ficaram obsoletos e foram reescritos (`collector/hardware_test.go`, `service/windows_test.go`) — a cobertura da cadeia de fallback em si migrou para `collector/session_test.go`, testada de forma pura/injetável. **Não coberto**: leitura/exibição do SID no backend (API de listagem, frontend) — fora de escopo desta correção, só persistência |
...
| B.13 ✅ | `KnownFields(true)` no parse do YAML | M12 | baixo | médio | Implementado e testado. Caso-limite tratado: `yaml.Decoder.Decode` em arquivo vazio devolve `io.EOF` (diferente de `yaml.Unmarshal`) — sem tratar isso à parte, um `agent.yaml` vazio passaria a falhar com "EOF" em vez da mensagem clara de `agent_key` ausente. **Risco real permanece**: um `agent.yaml` já implantado com algum campo digitado errado, hoje ignorado silenciosamente, passa a impedir o agente de iniciar |
...
| C.5 ✅ | Reduzir latência server-side do `machine-login` (paralelizar queries, `UpdateProfile` em background) | M9 | médio | médio | Implementado com `errgroup` — `UpdateProfile` e `AdminGenerateLink` rodam em paralelo (eram sequenciais; não têm dependência real entre si). **Não é "background" puro**: o handler espera as duas terminarem antes de responder, de propósito — função serverless da Vercel pode suspender a execução assim que a resposta é enviada, então uma goroutine verdadeiramente solta arriscaria ser morta no meio. **Sem teste automatizado**: `db` é `*pgxpool.Pool` direto, sem interface — testar exigiria Postgres real (mesma barreira do A.9); verificado por build+vet+revisão manual |
