...
(async () => {
  ...
  try {
    ...
    console.log("\n--- Test 6: Global Search Placeholder ---");
    const placeholder = await page.locator('input[placeholder*="Buscar tickets"]').getAttribute('placeholder');
    if (placeholder === "Buscar tickets por #número, título ou cliente...") {
      console.log("✅ Placeholder is complete: " + placeholder);
    } else {
      console.log("❌ Placeholder is incorrect: " + placeholder);
    }
    await takeScreenshot("test6_placeholder");
    ...
    await page.goto('http://localhost:4175/novo');
    await page.waitForLoadState('networkidle');
    
    await page.fill('input[name="titulo"]', '1234');
    ...
    await page.goto('http://localhost:4175/historico');
    await page.waitForLoadState('networkidle');
    
    await page.fill('input[placeholder*="Buscar"]', '1069');
    await page.waitForTimeout(1500);
    
    const countSingular = await page.locator('text="1 resultado"').count();
    if (countSingular > 0) {
      console.log("✅ Singular '1 resultado' found");
    } else {
      console.log("❌ Singular text not found.");
    }
    ...
    await page.goto('http://localhost:4175/monitoramento');
    await page.waitForLoadState('networkidle');
    
    const alertCount = await page.locator('text="offline"').count();
    if (alertCount > 0) {
