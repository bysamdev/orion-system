...
import { Clock, MessageSquare, RotateCcw, Timer, Lock, User, AlertCircle, Reply } from 'lucide-react';
...
import { ehReaberturaPeloSolicitante } from '@/lib/reaberturaAutomatica';
...
interface UnifiedTimelineProps {
  ...
  /** user_id do chamado — quem o abriu. Sem ele a reabertura automática não é destacada. */
  ...
  statusHistory.forEach(sh => {
    ...
    items.push({
      ...
      meta: {
        ...
        reaberturaAutomatica: ehReaberturaPeloSolicitante(sh, solicitanteId),
      }
    });
  });
...
const TimelineEntry: React.FC<{ item: TimelineItem; isLast: boolean }> = ({ item, isLast }) => {
  const reaberturaAutomatica = item.meta?.reaberturaAutomatica === true;

  const iconBg = reaberturaAutomatica ? 'bg-emerald-500/20'
    : item.isInternal ? 'bg-amber-500/20'
      ...
      <div className="flex-1 pb-6 min-w-0">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-1">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {reaberturaAutomatica && (
              <Badge variant="outline" className="bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-300 dark:border-emerald-700 text-xs">
                <Reply className="w-3 h-3 mr-1" />
                Reaberto pela resposta do solicitante
              </Badge>
            )}
        ...
        <p className={cn(
          ...
          item.isInternal
            ...
            : reaberturaAutomatica
              ? 'text-foreground font-medium bg-emerald-100/50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-3'
              : item.type === 'comment'
...
export const UnifiedTimeline: React.FC<UnifiedTimelineProps> = ({
  ...
  const allItems = buildTimeline(updates, statusHistory, timeEntries, profilesMap, solicitanteId);
