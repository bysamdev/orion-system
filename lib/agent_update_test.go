...
func TestLatestAgentVersionEmSincronia(t *testing.T) {
	t.Run("bate com orion-agent/version/version.go", func(t *testing.T) {
		...
		// var, não const: o valor é o default de fallback quando o binário é
		// compilado sem -ldflags -X (ver orion-agent/version/version.go) —
		// mas ainda é esse literal que precisa bater com LatestAgentVersion.
		m := regexp.MustCompile(`(?:const|var)\s+Version\s*=\s*"([^"]+)"`).FindSubmatch(fonte)
		if m == nil {
			t.Fatal("não encontrei `var Version = \"...\"` em orion-agent/version/version.go")
		}
